...
struct StudentInfo *deletestu(struct StudentInfo *head,int n,int m,int *count);				//函数声明
...
void main()
{
	...
	printf("学生的人数：%d\n",count1);
	head=deletestu(head,count1,M,&count2);		//删除
	count(head);							//计算综合成绩
	sort(head,count2);						//排序
	printf("可评奖学金学生的人数：%d\n",count2);
...
//	返回值：return head  链表的头指针
//========================================================
struct StudentInfo *deletestu(struct StudentInfo *head,int n,int m,int *count)
{
	FILE *fp;
	...
	{	
		...
		do
		{
			...
			if(i<n)
			{
				...
				printf("学号：%-12s姓名：%-12s德育成绩：%4d智育成绩：%4d体育成绩：%4d\n综合测评成绩：%.2lf\n",pper->xh,pper->xm,pper->dycj,pper->zycj,pper->tycj,pper->zhcjpji);
				i++;
			}
			else break;
			pper=pper->next;
