...
void read(struct Information *person,int n);
...
void main()
{
	...
	int count;										//分别定义男女平均年龄值double型
	double averf,averm;
					
	
	read(person,N);										//调用函数读出文件中的数据并将其存放在结构数组中
	print(person,N);									//调用函数将结构数组中的数据打印到屏幕上
	aver(person,&averf,&averm,N);							//调用函数求结构数组中男女的平均年龄
...
void aver(struct Information *person,double *paverf,double *paverm,int n)
{
	...
	*(paverf)=(sumf*1.0)/fnum;
	*(paverm)=(summ*1.0)/mnum;
